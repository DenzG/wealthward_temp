...
$curl = curl_init();
$url_path = "https://api.0x.org/swap/allowance-holder/price?chainId={$chainId}&sellToken={$sellToken}&buyToken={$buyToken}&sellAmount={$sellAmount}&taker={$taker}";
...
if ($httpCode !== 200) {
  ...
  echo json_encode(['error' => '0x price API failed', 'code' => $httpCode]);
} else {
  echo $response;
}
?>
